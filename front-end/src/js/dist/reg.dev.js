...
$('input.register').on('input focus', function (e) {
  ...
  return function (e) {
    if (e.type == 'focus') {
      $('.input-box').removeClass('active'); // console.log($(this));
...
function checkInput(e) {
  ...
  var con = $('#confirmed input').val();
  var checkTel = checkReg(/^1[3-9]\d{9}$/);
  var checkPassword = checkReg(/^[A-Z][a-zA-Z0-9]{5,15}$/); // 柯里化，检查密码和用户名

  if (name == 'account') {
    checkTel($('#account'));
  } else {
    checkPassword($('#password')); // 判断密码和确认密码内容是否一致
...
$('.login-btn').on('click', function (e) {
  if (isSame) {
    ...
    var inputs = $(this).parents('form').find('input.register');
    $.ajax({
      ...
      success: function success(data) {
        ...
        if (data.code == 1) {
          ...
          alert(data.mes); // 登录成功后，跳转到首页

          location.href = 'login.html';
        } else {
          alert(data.mes); // 重置input框信息
